           ...
           int debug)
{
  ...
  winTraceSetup("nekrs::setup: before THREAD MODEL selection");

  // precedence: cmd arg, par, env-var
  if (options->getArgs("THREAD MODEL").length() == 0) {
    const char *defaultMode = getenv("NEKRS_OCCA_MODE_DEFAULT");
    std::string value = defaultMode ? defaultMode : "SERIAL";
  ...
  winTraceSetup("nekrs::setup: before platform_t::getInstance");
  platform = platform_t::getInstance(*options, commg, comm);
  winTraceSetup("nekrs::setup: after platform_t::getInstance");
  platform->par = par;
