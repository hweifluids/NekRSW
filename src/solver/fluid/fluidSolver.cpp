...
void fluidSolver_t::solveVelocity(double time, int stage)
{
  ...
  const auto o_rhs = [&]() {
    ...
    launchKernel("fluidSolver_t::velocityNeumannBCHex3D",
                 ...
                 o_rho,
                 o_mue,
                 static_cast<occa::memory>(platform->app->bc->o_usrwrk),
                 o_Ue,
                 o_rhs);
...
void fluidSolver_t::applyDirichlet(double time)
{
  ...
  const auto neknekOffset = neknek ? neknek->intValOffset() : 0;
  auto o_neknekPoints = neknek ? neknek->o_pointMap() : platform->deviceMemoryPool.reserve<dlong>(1);
  auto o_neknekU = neknek ? neknek->getField(velocityName).o_intVal : platform->deviceMemoryPool.reserve<dfloat>(1);
