...
#ifdef _WIN32
      cache_dir + "/nek5000/" + session_in + ".dll";
#else
      cache_dir + "/nek5000/lib" + session_in + ".so";
#endif
...
void buildNekInterface(int ldimt, int N, int np, setupAide &options)
{
  ...
  const std::string installDir(getenv("NEKRS_HOME"));
  const std::string nek5000_dir = installDir + "/nek5000";
  const std::string cache_dir = (fs::path(getenv("NEKRS_CACHE_DIR")) / "nek5000").string();

  const std::string casename = options.getArgs("CASENAME");
  std::string usrFile = fs::absolute(fs::path(options.getArgs("NEK USR FILE"))).string();
  const std::string usrCaseName = casename + ".usr";
  const std::string usrFileCache = (fs::path(cache_dir) / fs::path(usrCaseName).filename()).string();
#ifdef _WIN32
  const std::string libFile = (fs::path(cache_dir) / (casename + ".dll")).string();
#else
  ...
  int err = [&]() {
    if (buildRank == 0) {
      ...
      if (buildRequired) {
        ...
        char buf[bufSize];
#ifdef _WIN32
        out_args = verbose ? "" : std::string("> \"") + makeOutput + "\" 2>&1";
        snprintf(buf,
                 bufSize,
                 "powershell -NoProfile -ExecutionPolicy Bypass "
                 "-File \"%s/bin/build-nek-interface.ps1\" "
                 "-CacheDir \"%s\" "
                 "-CaseDir \"%s\" "
                 "-CaseName \"%s\" "
                 "-InstallDir \"%s\" "
