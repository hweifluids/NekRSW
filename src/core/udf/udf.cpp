...
void udfBuild(setupAide &options)
{
  ...
  if (platform->cacheBcast || platform->cacheLocal) {
    if (oudfFileExists) {
      ...
      oudfFile = (fs::path(platform->tmpDir) / fs::path(oudfFile).filename()).string();
...
void *udfLoadFunction(const char *fname, int errchk)
{
  if (!libudfHandle) {
    ...
    if (platform->cacheBcast) {
      cache_dir = fs::path(platform->tmpDir).string();
    }

#ifdef _WIN32
    const auto udfLib = (fs::path(cache_dir) / "udf/libudf.dll").string();
#else
    const auto udfLib = (fs::path(cache_dir) / "udf/libudf.so").string();
#endif
