...
{
  const std::string includeFile = (fs::path(udfFileCache).parent_path() / fs::path("udfAutoLoadKernel.hpp")).string();
...
int udfMake(setupAide &options, const std::string &solverName, int rank)
{
  ...
  udfFile = fs::absolute(udfFile).string();
...
#ifdef _WIN32
  const std::string udfLib = cache_dir + "/udf/libudf.dll";
#else
  ...
  const std::string case_dir = fs::current_path().string();
...
#ifdef _WIN32
  const std::string libnekrsFile = installDir + "/bin/nekrs.dll";
#else
  ...
  if (!oudfFile.empty()) {
    oudfFileExists = 1;
    oudfFile = fs::absolute(oudfFile).string();
  }

  auto tStart = std::chrono::high_resolution_clock::now();

  const int cmdSize = 8192;
  char cmd[cmdSize];
  ...
  fs::remove(cache_dir + "/udf/okl.cpp");
#ifdef _WIN32
  for (const auto &entry : fs::directory_iterator(cache_dir + "/udf")) {
    const auto path = entry.path();
    if (path.filename().string().rfind("okl.cpp.", 0) == 0) {
      fs::remove(path);
    }
  }
#endif
...
#ifdef _WIN32
  auto envOr = [](const char *name, const char *fallback) {
    const char *value = getenv(name);
    return (value && *value) ? std::string(value) : std::string(fallback);
  };
  const std::string cmakeCxx = envOr("NEKRS_CXX", "cl");
  const std::string cmakeCxxFlags = envOr("NEKRS_CXXFLAGS", "");
  const std::string cmakeLog = cmakeBuildDir + "/cmake.log";
  fs::remove(cmakeBuildDir + "/CMakeCache.txt");
  fs::remove_all(cmakeBuildDir + "/CMakeFiles");
  snprintf(cmd,
           cmdSize,
           "cmake %s -S \"%s\" -B \"%s\" "
           "-DNEKRS_USE_DFLOAT_FLOAT=%s "
           "-DCMAKE_BUILD_TYPE=RelWithDebInfo "
           "-DNEKRS_INSTALL_DIR=\"%s\" -DCASE_DIR=\"%s\" -DCMAKE_CXX_COMPILER=\"%s\" "
           "-DCMAKE_CXX_FLAGS=\"%s\" -DCMAKE_VERBOSE_MAKEFILE=%s > \"%s\" 2>&1",
           cmakeFlags.c_str(),
           cmakeBuildDir.c_str(),
           cmakeBuildDir.c_str(),
           useFloat.c_str(),
...
#ifdef _WIN32
  fs::remove(cmakeBuildDir + "/libudf.dll");
#else
...
#ifdef _WIN32
    fs::create_directories(fs::path(postOklSource).parent_path());
    fs::copy(cmakeBuildDir + "/okl.cpp", postOklSource, fs::copy_options::overwrite_existing);
#else
  ...
  { // build
#ifdef _WIN32
    const std::string buildLog = cmakeBuildDir + "/cmake.log";
    const std::string buildStdoutFlag = (verbose) ? std::string("") : std::string(">> \"") + buildLog + "\" 2>&1";
    snprintf(cmd, cmdSize, "cmake --build \"%s\" --target udf --config Debug %s", cmakeBuildDir.c_str(), buildStdoutFlag.c_str());
#else
