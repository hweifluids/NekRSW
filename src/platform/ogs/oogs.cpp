...
static void neighborAllToAll(int unit_size, oogs_t *gs)
{
...
#ifdef _WIN32
  std::vector<MPI_Request> requests(pwd->comm[recv].n + pwd->comm[send].n);
  int requestCount = 0;
  for (uint i = 0; i < pwd->comm[recv].n; ++i) {
    MPI_CHECK(MPI_Irecv(bufRecv + gs->nbc.recvdispls[i],
                        gs->nbc.recvcounts[i],
                        MPI_UNSIGNED_CHAR,
                        pwd->comm[recv].p[i],
                        0,
  ...
  for (uint i = 0; i < pwd->comm[send].n; ++i) {
    MPI_CHECK(MPI_Isend(bufSend + gs->nbc.senddispls[i],
                        gs->nbc.sendcounts[i],
                        MPI_UNSIGNED_CHAR,
                        pwd->comm[send].p[i],
                        0,
