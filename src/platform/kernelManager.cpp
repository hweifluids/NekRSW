...
void kernelManager_t::compile()
{
  ...
  auto compile = [&](const kernelRequest_t &req) {
    ...
    try {
      ...
      if (platform->verbose() || platform->buildOnly) {
        auto binaryFilename = fs::path(getenv("OCCA_CACHE_DIR")) / "cache" / hash / "binary";
#ifdef _WIN32
        if (!fs::exists(binaryFilename)) {
          auto dllFilename = binaryFilename;
          dllFilename += ".dll";
          if (fs::exists(dllFilename)) {
            binaryFilename = dllFilename;
          }
        }
#endif
    ...
    const auto duplicateHashFound = [&]() {
      if (platform->comm.mpiRank() == 0) {
        ...
        for (const auto &req : requests) {
          ...
          const std::string hash(hashes + reqId * hashLength);
  ...
  if (platform->cacheBcast && !platform->buildOnly) {
    ...
    const std::string cacheDir = (fs::path(platform->tmpDir) / fs::path("occa/")).string();
