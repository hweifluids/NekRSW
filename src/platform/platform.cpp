...
static void winTraceSetup(const char *message)
{
#ifdef _WIN32
  if (getenv("NEKRS_WIN_TRACE_SETUP")) {
    fprintf(stderr, "[nekrs-win] %s\n", message);
    fflush(stderr);
  }
#else
  (void)message;
...
{
  winTraceSetup("platform_t::platform_t: constructor body entered");
  int rank;
  ...
  cacheLocal = 0;
  const char *cacheLocalEnv = getenv("NEKRS_CACHE_LOCAL");
  if (cacheLocalEnv && *cacheLocalEnv) {
    cacheLocal = std::stoi(cacheLocalEnv);
  }

  cacheBcast = 0;
  const char *cacheBcastEnv = getenv("NEKRS_CACHE_BCAST");
  if (cacheBcastEnv && *cacheBcastEnv) {
    cacheBcast = std::stoi(cacheBcastEnv);
  }
  ...
  oogs::gpu_mpi((gpuMpiEnv && *gpuMpiEnv) ? std::stoi(gpuMpiEnv) : 0);
  oogs::overlap(options.compareArgs("ENABLE GS COMM OVERLAP", "FALSE") ? 0 : 1);

  const char *syncRecvEnv = getenv("OOGS_SYNC_RECV");
  if (syncRecvEnv && *syncRecvEnv) {
    oogs::sync_recv(std::stoi(syncRecvEnv));
  }
  winTraceSetup("platform_t::platform_t: after OGS options");
  ...
  if (cacheBcast || cacheLocal) {
    ...
    if (multiSession) {
      ...
      tmpDir = (fs::path(tmpDir) / fs::path("sess" + std::to_string(sessionID))).string();
...

  {
    ...
    if (rankLocal == 0) {
      ...
      const char *cacheDirEnv = getenv("NEKRS_CACHE_DIR");
      cache_dir.assign((cacheDirEnv && *cacheDirEnv) ? cacheDirEnv : ".cache");
#ifdef _WIN32
  ...
  winTraceSetup("platform_t::platform_t: after cache directory setup");
  ...
  winTraceSetup("platform_t::platform_t: before compileDummyKernel");
  if (rank == 0) {
    compileDummyKernel(*this);
  }
  winTraceSetup("platform_t::platform_t: after compileDummyKernel");
...
void platform_t::bcastJITKernelSourceFiles()
{
  ...
  setenv("NEKRS_HOME", NEKRS_HOME_NEW.string().c_str(), 1);
  setenv("NEKRS_KERNEL_DIR", (NEKRS_HOME_NEW / "kernels").string().c_str(), 1);
}
