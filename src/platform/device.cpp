...
fs::path occaCachedBinaryPath(const std::string &hash)
{
  fs::path binaryFilename = fs::path(getenv("OCCA_CACHE_DIR")) / "cache" / hash / "binary";
#ifdef _WIN32
  if (!fs::exists(binaryFilename)) {
    fs::path dllFilename = binaryFilename;
    dllFilename += ".dll";
    if (fs::exists(dllFilename)) {
      return dllFilename;
    }
  }
#endif
                                     ...
                                     const MPI_Comm &commIn) const
{
  ...
  const auto binaryFilename = occaCachedBinaryPath(hash);
  nekrsCheck(!fs::exists(binaryFilename),
             comm,
             EXIT_FAILURE,
             "cannot find kernel binary <%s>!\n",
             binaryFilename.string().c_str());
...
{
  winTraceSetup("device_t::device_t: enter");
  // OCCA build stuff
  constexpr int deviceConfigSize = 4096;
  ...
  winTraceSetup("device_t::device_t: after THREAD MODEL read");
...
#if 1
  winTraceSetup("device_t::device_t: before verbose settings");
...
#endif
  winTraceSetup("device_t::device_t: before setOccaVars");
  setOccaVars();
  ...
  winTraceSetup("device_t::device_t: before occa device setup");
  this->_device.setup(std::string(deviceConfig));
  winTraceSetup("device_t::device_t: after occa device setup");
  ...
  if (this->mode() == "Serial") {
    deviceAtomic = false;
    winTraceSetup("device_t::device_t: skipped atomicsAvailable for Windows Serial backend");
  } else
#endif
  {
    winTraceSetup("device_t::device_t: before atomicsAvailable");
    deviceAtomic = atomicsAvailable(*this, _comm.mpiComm());
  }
  winTraceSetup("device_t::device_t: leave");
}
