...
static int balance_partitions(struct array *elements, unsigned nv,
  ...
  comm_allreduce(lc, gs_slong, gs_add, &nelgt, 1, &wrk);
  comm_allreduce(gc, gs_slong, gs_add, &nglob, 1, &wrk);
  ...
  if (send_cnt > 0) {
    ...
    comm_scan(out, lc, gs_slong, gs_add, &nielems, 1, bfr);
    slong start = out[0][0];
...
void rsb(struct array *elements, int nv, const parrsb_options *const options,
  ...
  for (uint level = 0; level < levels; level++) {
    ...
    for (uint cut = 0; cut < ncuts; cut++) {
      ...
      case 0:
        parallel_sort(struct rsb_element, elements, globalId, gs_slong, 0, 1,
                      &lc, bfr);
        break;
