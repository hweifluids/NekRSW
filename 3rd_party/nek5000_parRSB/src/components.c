...
uint get_components(sint *component, struct array *elems, unsigned nv,
  ...
  do {
    ...
    if (bin == 1) {
      ...
      slong mfirst = first;
      comm_allreduce(&cc, gs_slong, gs_min, &mfirst, 1, wrk);
      ...
      do {
        gs(p, gs_slong, gs_add, 0, gsh, buf);
        ...
        comm_allreduce(&cc, gs_slong, gs_add, &nnzg, 1, &nnzb);
      } while (nnzg > nnzg0);
      gs_free(gsh);
    }
    comm_free(&cc);

    comm_allreduce(c, gs_slong, gs_add, &nnz1, 1, &nnzb);
    nmarked += nnz1, count++;
  } while (nmarked < nelg);
...
uint get_components_v2(sint *component, struct array *elems, unsigned nv,
  ...
  slong out[2][1], wrk[2][1], in = nelt;
  comm_scan(out, ci, gs_slong, gs_add, &in, 1, wrk);
  ulong nelg = out[1][0];
  ...
  do {
    ...
    if (bin == 1) {
      ...
      do {
        ...
        nnzg = nnz;
        comm_allreduce(&c, gs_slong, gs_add, &nnzg, 1, wrk);
    ...
    comm_allreduce(ci, gs_slong, gs_max, &nnzg, 1, &wrk);
    nmkd += nnzg;
    comm_allreduce(ci, gs_slong, gs_max, &ncg, 1, &wrk);
    nc += ncg;
  } while (nmkd < nelg);
