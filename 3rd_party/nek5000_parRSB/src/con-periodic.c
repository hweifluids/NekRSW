...
static int compressPeriodicVertices(Mesh mesh, struct comm *c, buffer *bfr) {
  parallel_sort(struct point_t, &mesh->elements, globalId, gs_slong, 0, 0, c,
                bfr);
  ...
  if (npoints > 0)
    ...
    in[0] = 0;
  comm_scan(out, c, gs_slong, gs_add, in, 1, buf);
  slong start = out[0][0];
...
static int renumberPeriodicVertices(Mesh mesh, struct comm *c,
  ...
  gs(mnew, gs_slong, gs_min, 0, gsh, bfr);
  ...
  do {
    ...
    gs(mnew, gs_slong, gs_min, 0, gsh1, bfr);
    gs_free(gsh1);
...
int match_periodic_faces(Mesh mesh, struct comm *c, int verbose, buffer *bfr) {
  ...
  comm_allreduce(c, gs_slong, gs_add, &n, 1, &wrk);
