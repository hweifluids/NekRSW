...
double get_scalar(struct array *a, uint i, uint offset, uint usize,
  ...
  case gs_slong: data = *((ulong *)v); break;
...
void sarray_transfer_chunk(struct array *arr, const size_t usize,
  ...
  comm_allreduce(c, gs_slong, gs_add, &ng, 1, wrk);
  if (ng == 0) return;
  ...
  for (unsigned t = 0; t < nt; t++) {
    ...
    comm_allreduce(c, gs_slong, gs_max, &cmax, 1, wrk);
    comm_allreduce(c, gs_slong, gs_max, &bmax, 1, wrk);
    comm_allreduce(c, gs_slong, gs_min, &cmin, 1, wrk);
    comm_allreduce(c, gs_slong, gs_min, &bmin, 1, wrk);
