...
#if (OCCA_OS == OCCA_WINDOWS_OS)
      std::error_code ec;
      for (const auto &entry : std::filesystem::directory_iterator(expDir, ec)) {
        if (ec) {
          return files;
        }

        const bool wantDir = (fileType == DT_DIR);
        const bool wantReg = (fileType == DT_REG);
        if ((wantDir && !entry.is_directory()) || (wantReg && !entry.is_regular_file())) {
          continue;
        }

        std::string fullname = convertSlashes(entry.path().string());
        if (wantDir) {
...
#if (OCCA_OS == OCCA_WINDOWS_OS)
      const std::string expFilename = io::expandFilename(filename);
      const int fd = _open(expFilename.c_str(), _O_RDWR | _O_BINARY);
      if (fd >= 0) {
        _commit(fd);
        _close(fd);
      }
