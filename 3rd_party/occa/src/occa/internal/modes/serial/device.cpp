...
namespace occa {
  namespace serial {
    namespace {
      ...
      void addWindowsRawKernelExports(const std::string &filename) {
#if (OCCA_OS == OCCA_WINDOWS_OS)
    ...
    modeKernel_t* device::buildKernel(const std::string &filename,
      ...
      winTrace("serial::device::buildKernel: enter compile path");
      ...
      winTrace("serial::device::buildKernel: before compilerVendor");
      const int compilerVendor = sys::compilerVendor(compiler);
      winTrace("serial::device::buildKernel: after compilerVendor");
...
#  endif
      ...
      winTrace("serial::device::buildKernel: after language flags");
      ...
      if (isLauncherKernel) {
        ...
        // Cache raw origin
        winTrace("serial::device::buildKernel: before cacheFile");
        sourceFilename = (
        ...
        winTrace("serial::device::buildKernel: after cacheFile");

        if (compilingOkl) {
          const std::string outputFile = hashDir + kc::cachedSourceFilename(filename);
          winTrace("serial::device::buildKernel: before parseFile");
          bool valid = parseFile(sourceFilename,
                                 outputFile,
                                 kernelProps,
                                 metadata);
          winTrace("serial::device::buildKernel: after parseFile");
          if (!valid) {
            return NULL;
          }
        ...
        if (!compilingOkl) {
          addWindowsRawKernelExports(sourceFilename);
        }
      }
      ...
      winTrace("serial::device::buildKernel: before stageFile");
      io::stageFile(
        binaryFilename,
        true,
        [&](const std::string &tempFilename) -> bool {
          ...
          command << ' '    << compilerLinkerFlags
                  ...
                  << std::endl;
#else
          command << quoteForWindowsShell(compiler)
                  << " /D MC_CL_EXE"
                  ...
                  << ' '       << compilerFlags;
          if (includeOcca) {
            command << " /I"     << quoteForWindowsShell(env::OCCA_DIR + "include")
                    << " /I"     << quoteForWindowsShell(env::OCCA_INSTALL_DIR + "include");
          }
          command << " /Fo"    << quoteForWindowsShell(tempFilename + ".obj")
                  << ' '       << quoteForWindowsShell(sourceFilename)
                  << " /link";
          if (linkOcca) {
            command << ' '     << quoteForWindowsShell(env::OCCA_INSTALL_DIR + "lib/occa.lib");
          }
          command << ' '       << compilerLinkerFlags
                  << (!compilingOkl && kernelName.size() ? (" /EXPORT:" + kernelName) : "")
                  << " /OUT:"  << quoteForWindowsShell(tempFilename)
                  << std::endl;
#endif

          const std::string &sCommand = strip(command.str());
          winTrace("serial::device::buildKernel: command: " + sCommand);
          if (verbose) {
          ...
          if (commandExitCode) {
            winTrace("serial::device::buildKernel: compiler command failed");
            OCCA_FORCE_ERROR(
      ...
      winTrace("serial::device::buildKernel: after stageFile");
