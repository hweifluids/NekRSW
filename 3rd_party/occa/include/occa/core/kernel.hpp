...
#include "codegen/kernelOperators.hpp_codegen"
    ...
    void operator () (Args&&... args) const {
      run({toKernelArg(std::forward<Args>(args))...});
    }
  ...
  private:
    static kernelArg toKernelArg(const kernelArg &arg) {
      return arg;
    }

    static kernelArg toKernelArg(const memory &arg) {
      return kernelArg(arg);
    }

    template <class T>
    static kernelArg toKernelArg(const T &arg) {
      return kernelArg(arg);
    }
  };
