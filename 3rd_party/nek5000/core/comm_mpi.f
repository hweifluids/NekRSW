...
c---------------------------------------------------------------------
     ...
     $   call exitti('Single precision mode not supported!',wdsize)
      ...
      call MPI_Type_size(MPI_DOUBLE_PRECISION,isize_mpi_i,ierr)
      isize_mpi = isize_mpi_i
      if (isize_mpi .ne. wdsize) then
         call exitti('MPI real size does not match$',isize_mpi)
      endif

      call MPI_Type_size(MPI_INTEGER,isize_mpi_i,ierr)
      isize_mpi = isize_mpi_i
       if (isize_mpi .ne. isize) then
         call exitti('MPI integer size does not match$',isize_mpi)
      endif

      call MPI_Type_size(MPI_INTEGER8,isize_mpi_i,ierr)
      isize_mpi = isize_mpi_i
...
c-----------------------------------------------------------------------
      ...
      character*25 s25
      integer lstr,leno,last
      include 'SIZE'
      include 'TOTAL'
      include 'CTIMER'

      stringo = ' '
      lstr = min(len(stringi),132)
      if (lstr.gt.0) stringo(1:lstr) = stringi(1:lstr)
      leno = index(stringo,'$')
      if (leno.eq.0) leno = min(lstr+1,132)
      write(s25,25) rdata,idata
   25 format(1x,1p1e14.6,i10)
      if (leno.le.132) stringo(leno:) = s25

      last = min(132,leno+24)
      if (nid.eq.0) write(6,1) stringo(1:last)
    1 format('EXIT: ',a)
...
c-----------------------------------------------------------------------
      ...
      write(s11,11) idata
   11 format(1x,i10)
      if (leno.le.132) stringo(leno:) = s11

      last = min(132,leno+10)
      if (nid.eq.0) write(6,1) stringo(1:last)
    1 format('EXIT: ',a)
